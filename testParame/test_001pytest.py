...
from baseIni import BaseConfig
...
# Setup the desired capabilities for the Android device and the application
...
def app_driver(request):
    config = BaseConfig()
    driver = webdriver.Remote(config.get_appium_server_url(), config.get_desired_capabilities())
